...
export function MindMapNode({
  ...
  const handleAddChild = () => {
    setShowContextMenu(false);
    window.dispatchEvent(
      new CustomEvent('mindmap-add-child', {
        detail: { nodeId: id },
      })
    );
  };
  ...
  const handleAddSibling = () => {
    setShowContextMenu(false);
    window.dispatchEvent(
      new CustomEvent('mindmap-add-sibling', {
        detail: { nodeId: id },
      })
    );
  };
  ...
  const handleDelete = () => {
    ...
    if (!data.isRoot) {
      window.dispatchEvent(
        new CustomEvent('mindmap-delete-node', {
          detail: { nodeId: id },
        })
      );
    }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {((data.sources && data.sources.length > 0) || data.notes?.trim()) && (
