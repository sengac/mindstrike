...
export class MCPManager extends EventEmitter {
  ...
  private isValidServerConfig(config: unknown): config is MCPServerConfig {
    return (
      ...
      'id' in config &&
      'command' in config &&
      typeof (config as MCPServerConfig).id === 'string' &&
      typeof (config as MCPServerConfig).command === 'string'
    );
  }
  ...
  private async connectToEnabledServers(): Promise<void> {
    ...
    for (const server of enabledServers) {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Unknown error';
        this.logMCP(server.id, 'error', `Failed to connect: ${errorMessage}`);
      }
    }
  }
    ...
    try {
      if (processedConfig.transport === 'sse' && processedConfig.url) {
        ...
        transport.onmessage = (message: unknown) => {
          ...
          function isMCPMessage(obj: unknown): obj is MCPMessage {
          ...
          if (originalOnMessage) {
            originalOnMessage.call(
              transport,
              message as Parameters<typeof originalOnMessage>[0]
            );
          }
        };
  ...
  async disconnectFromServer(serverId: string): Promise<void> {
    ...
    if (client) {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Unknown error';
        this.logMCP(serverId, 'error', `Error disconnecting: ${errorMessage}`);
      }
    }
  }
          ...
          func: async (input: unknown) => {
            try {
              ...
              this.logMCP(
                serverId,
                'error',
                `LangChain tool wrapper error for ${toolName}: ${errorMessage}`
              );
  ...
  private convertToZodSchema(inputSchema: unknown): z.ZodType<unknown> {
    if (!inputSchema || typeof inputSchema !== 'object') {
      return z.object({}).optional();
    }

    try {
      // Type guard for JSON Schema object
      ...
      if (inputSchema.type === 'object' && inputSchema.properties) {
        const zodObj: Record<string, z.ZodType<unknown>> = {};
        ...
        for (const [key, prop] of Object.entries(inputSchema.properties)) {
          ...
          function isPropSchema(obj: unknown): obj is PropSchema {
            return typeof obj === 'object' && obj !== null;
          }

          if (!isPropSchema(prop)) {
            zodObj[key] = z.any();
            continue;
          }
    ...
    try {
      ...
      for (const [id, server] of this.servers.entries()) {
        ...
        if (server.transport && server.transport !== 'stdio') {
          serverConfig.transport = server.transport;
        }
        if (server.url) {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logMCP('manager', 'error', `Failed to save config: ${errorMessage}`);
      throw error;
    }
  }
  ...
  async reload(): Promise<void> {
    try {
      ...
      this.logMCP(
        'manager',
        'error',
        `Failed to reload configuration: ${errorMessage}`
      );
      throw error;
    }
  }
