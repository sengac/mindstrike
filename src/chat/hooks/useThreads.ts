...
export function useThreads() {
  ...
  const loadThreads = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        const parsedThreads = data.map(
          (thread: {
            createdAt: string;
            updatedAt: string;
            messages: Array<{ timestamp?: string; [key: string]: unknown }>;
          }) => ({
            ...thread,
        ...
        if (parsedThreads.length > 0) {
          const mostRecent = parsedThreads.sort(
            (a: Thread, b: Thread) =>
  ...
  const updateThreadRole = useCallback(
    async (threadId: string, customRole?: string) => {
      const updatedThreads = threads.map(thread =>
        thread.id === threadId
      ...
      setTimeout(async () => {
        try {
          ...
          const userMsg = thread.messages.find(
            (m: { role: string; [key: string]: unknown }) => m.role === 'user'
          );
          if (!userMsg) return;
          ...
          if (titleResponse.ok) {
            ...
            if (title && title.length > 0) {
              ...
              const updatedThreads = latestThreads.map(
                (t: {
                  id: string;
                  name?: string;
                  updatedAt: string;
              ...
              // Update local state
              const formattedThreads = updatedThreads.map(
                (thread: {
                  createdAt: string;
                  updatedAt: string;
                  messages: Array<{
  ...
  const updateThreadMessages = useCallback(
    async (threadId: string, messages: ConversationMessage[]) => {
  ...
  const deleteMessage = useCallback(
    async (threadId: string, messageId: string) => {
      ...
      const updatedMessages = thread.messages.filter(
