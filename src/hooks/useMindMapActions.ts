...
interface UseMindMapActionsProps {
  ...
  generateEdges: (nodes: Node<MindMapNodeData>[], layout: 'LR' | 'RL' | 'TB' | 'BT') => any[]
  arrangeNodes: (nodes: Node<MindMapNodeData>[], edges: any[], rootId: string, layout?: 'LR' | 'RL' | 'TB' | 'BT') => Node<MindMapNodeData>[]
  updateNodeLevels: (nodes: Node<MindMapNodeData>[], edges: any[], rootId: string, layout: 'LR' | 'RL' | 'TB' | 'BT') => Node<MindMapNodeData>[]
  saveToHistory: (nodes: Node<MindMapNodeData>[], rootId: string, layout?: 'LR' | 'RL' | 'TB' | 'BT') => void
  convertNodesToTree: (nodes: Node<MindMapNodeData>[], rootId: string, layout: 'LR' | 'RL' | 'TB' | 'BT') => MindMapData
...
export function useMindMapActions({
  ...
  const applyLayoutAndSave = useCallback(async (
    ...
    const finalNodes = updateNodeLevels(arrangedNodes, generatedEdges, rootNodeId, layout)
    ...
    generateEdges, arrangeNodes, updateNodeLevels, setNodes, setEdges,
    saveToHistory, convertNodesToTree, onSave, onLayoutComplete, layout, rootNodeId, setIsLoading
  ])
  ...
  // Add a sibling node
  const addSiblingNode = useCallback(async (siblingNodeId: string) => {
  ...
  const deleteNode = useCallback(async (nodeIdToDelete: string) => {
  ...
  const toggleNodeCollapse = useCallback(async (nodeId: string) => {
    const updatedNodes = nodes.map(node =>
      node.id === nodeId
        ? {
            ...node,
  ...
  const changeLayout = useCallback(async (newLayout: 'LR' | 'RL' | 'TB' | 'BT') => {
    const generatedEdges = generateEdges(nodes, newLayout)
    const arrangedNodes = arrangeNodes(nodes, generatedEdges, rootNodeId, newLayout)
    const finalNodes = updateNodeLevels(arrangedNodes, generatedEdges, rootNodeId, newLayout)
    ...
    if (onSave) {
      setTimeout(() => {
        const treeData = convertNodesToTree(finalNodes, rootNodeId, newLayout)
        onSave(treeData)
      }, 100)
    }
  }, [
    nodes, rootNodeId, generateEdges, arrangeNodes, updateNodeLevels,
    setNodes, setEdges, saveToHistory, convertNodesToTree, onSave, onLayoutComplete, setIsLoading
  ])
