...
async function demonstrateAgenticWorkflow() {
  ...
  const testQuery = "Add information about machine learning including supervised learning, unsupervised learning, and deep learning. Include examples and code snippets for each type.";
  ...
  const onUpdate = (update) => {
    ...
    console.log('📊 Progress Update:', update.type, '-', update.data.message || update.data.description);
    
    if (update.type === 'progress_update' && update.data.tasks) {
      console.log('📋 TODO List:');
      update.data.tasks.forEach((task, i) => {
        const status = task.status === 'todo' ? '⏳' : task.status === 'in-progress' ? '🔄' : '✅';
        console.log(`  ${i + 1}. ${status} ${task.description} (${task.priority})`);
      });
      console.log('---');
    }
  };
  ...
  try {
    ...
    const result = await agent.processMessageAgentic(testQuery, [], [], onUpdate);
    ...
    console.log(`- Tasks completed: ${parsed.workflow.tasksCompleted}/${parsed.workflow.totalTasks}`);
