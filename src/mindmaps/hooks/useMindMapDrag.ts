...
export function useMindMapDrag({
  ...
  const [closestDropTarget, setClosestDropTarget] = useState<string | null>(null)
  const [dropPosition, setDropPosition] = useState<'above' | 'below' | 'over' | null>(null)
  ...
  const handleSiblingPositioning = useCallback((
    ...
    const siblingNodes = nodes.filter(n => 
      n.data.parentId === parentNodeId && n.id !== nodeId
    )
    ...
    const targetSiblingIndex = siblingNodes.findIndex(n => n.id === targetNodeId)
  ...
  }, [rootNodeId, nodes, findClosestNode, handleSiblingPositioning, wouldCreateCycle, moveNode])
  ...
  const onNodeDrag: NodeDragHandler = useCallback((event, node) => {
    ...
    if (distance > 20) {
      ...
      if (closestNodeId && !wouldCreateCycle(node.id, closestNodeId)) {
        ...
        if (closestNodeId !== closestDropTarget || position !== dropPosition) {
    ...
    rootNodeId, draggedNodeId, dragStartPosition, hasDraggedSignificantly,
    closestDropTarget, dropPosition, findClosestNode, wouldCreateCycle, getDropPosition
  ])
