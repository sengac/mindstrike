  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 border border-gray-600 rounded-lg text-sm text-gray-200 transition-colors min-w-[180px] justify-between"
          ...
          {models.map((model) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-700 transition-colors border-b border-gray-700 last:border-b-0 ${
                selectedModel?.serviceId === model.serviceId && selectedModel?.model === model.model
            ...
            >
              <div className="flex flex-col gap-0.5">
                ...
                <div className="text-xs text-gray-400 truncate">{model.serviceName}</div>
              </div>
            </button>
          ))}
        </div>
