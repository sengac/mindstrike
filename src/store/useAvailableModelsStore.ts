...
export const useAvailableModelsStore = create<AvailableModelsState>((set, get) => ({
  ...
  performSearch: async (searchType: string = 'all') => {
    ...
    try {
      ...
      toast.success(`Found ${data.models?.length || 0} models for "${searchQuery.trim()}" in ${searchTypeText}`);
    } catch (error) {
      ...
      if (errorMessage.includes('timed out') || errorMessage.includes('504')) {
        toast.error('Search timed out. HuggingFace API is slow. Try a more specific search term.');
      } else if (errorMessage.includes('temporarily unavailable') || errorMessage.includes('502')) {
        toast.error('HuggingFace API is temporarily unavailable. Please try again later.');
      } else {
        toast.error('Failed to search models. Try again or search for different terms.');
      }
    } finally {
  ...
  loadAvailableModels: async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to load available models:', response.statusText);
      }
    } catch (error) {
  ...
  getDisplayModels: () => {
    const { hasSearched, searchQuery, searchResults, availableModels } = get();
    return hasSearched && searchQuery.trim() ? searchResults : availableModels;
  },
