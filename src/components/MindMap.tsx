...
function MindMapInner({
  ...
  const [generationSummary, setGenerationSummary] = useState<string | null>(
    null
  );
  ...
  const {
    ...
    stats,
    startStreaming,
    cancelGeneration: cancelStreamGeneration,
  ...
  if (!actionsManagerRef.current) {
    actionsManagerRef.current = new MindMapActionsManager(
    ...
    (updates: {
      ...
      React.startTransition(() => {
        ...
        if (updates.rootNodeId !== undefined) {
          setRootNodeId(updates.rootNodeId);
        }
        if (updates.layout !== undefined) {
          setLayout(updates.layout);
        }
        if (updates.selectedNodeId !== undefined) {
          setSelectedNodeId(updates.selectedNodeId);
        }
      });
    },
    []
  ...
  useEffect(() => {
    const handleExternalUpdates = async () => {
      ...
      if (externalNodeUpdates) {
        ...
        if (chatId !== undefined) {
          const newNodes = actionsManager.updateNodeChatId(
            currentNodes,
            nodeId,
            chatId
          );
        ...
        if (notes !== undefined) {
          const newNodes = actionsManager.updateNodeNotes(
            updatedNodes,
            nodeId,
            notes
          );
        ...
        if (sources !== undefined) {
          const newNodes = actionsManager.updateNodeSources(
            updatedNodes,
            nodeId,
            sources
          );
        ...
        if (hasChanges) {
          ...
          try {
            if (
              currentRootId &&
              updatedNodes.find(n => n.id === currentRootId)
            ) {
  ...
  const saveData = useCallback(async () => {
    ...
    if (
      ...
      currentRootId &&
      !isCurrentlyInitializing.current
    ) {
      try {
        const treeData = dataManager.convertNodesToTree(
          currentNodes,
  ...
  const handleUpdateNodeChatId = useCallback(
  ...
  const handleUpdateNodeNotes = useCallback(
    (nodeId: string, notes: string | null) => {
      const {
        nodes: currentNodes,
        rootNodeId: currentRootId,
        layout: currentLayout,
  ...
  const handleUpdateNodeSources = useCallback(
    (nodeId: string, sources: Source[]) => {
      const {
        nodes: currentNodes,
        rootNodeId: currentRootId,
        layout: currentLayout,
  ...
  const applyMindmapChanges = useCallback(
    async (changes: any[]) => {
      if (
        !dataManagerRef.current ||
        !layoutManagerRef.current ||
        !actionsManagerRef.current
      ) {
        return;
      }
      ...
      updatedNodes.forEach(node => {
        node.data.hasChildren = updatedNodes.some(
      ...
      const treeData = dataManagerRef.current.convertNodesToTree(
        result.nodes,
        rootNodeId,
        layout
      );
  ...
  useEffect(() => {
    ...
    const unprocessedTasks = currentWorkflow.tasks.filter(
      task =>
  ...
  const handleGenerate = useCallback(async () => {
    if (!generativeInput.trim() || !selectedNodeId || isGenerating) {
      return;
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      'mindmap-add-child',
      handleAddChild as EventListener
    );
    window.addEventListener(
      'mindmap-add-sibling',
      handleAddSibling as EventListener
    );
    window.addEventListener(
      'mindmap-delete-node',
  ...
  const keyboardHandlers = useMemo(() => {
      ...
      handleKeyDown: (e: KeyboardEvent) => {
        ...
        if (
          ...
          (matchesKeyBinding(e, getKeyBinding('deleteNode', 'Delete')) ||
            matchesKeyBinding(
              e,
              getKeyBinding('deleteNodeBackspace', 'Backspace')
            )) &&
          selectedNodeId
        ) {
          e.preventDefault();
          ...
          matchesKeyBinding(e, getKeyBinding('redo', 'Ctrl+Shift+Z'))
        ) {
          e.preventDefault();
          handleRedo();
          ...
          matchesKeyBinding(e, getKeyBinding('openGenerative', '/')) &&
          selectedNodeId
        ) {
          e.preventDefault();
    ...
    handleUndo,
    handleRedo,
    setIsAnimatingToPanel,
    setShowGenerativePanel,
  ]);

  useEffect(() => {
    window.addEventListener('keydown', keyboardHandlers.handleKeyDown);
    return () =>
      window.removeEventListener('keydown', keyboardHandlers.handleKeyDown);
      ...
      onNodeClick: (event: React.MouseEvent | React.TouchEvent, node: Node) => {
        ...
        if (
          ...
          setTimeout(() => {
            ...
            const anyContextMenuOpen = document.querySelector(
              '[role="menu"], .context-menu, [data-context-menu]'
            );
            if (!anyContextMenuOpen) {
              ...
              window.dispatchEvent(
                new CustomEvent('mindmap-close-context-menu')
              );
            }
          }, 150);
          return;
  ...
  useEffect(() => {
    ...
    if (
      newCanUndo !== undoRedoState.canUndo ||
      newCanRedo !== undoRedoState.canRedo
    ) {
  ...
  const draggedNode = useMemo(
    () =>
      draggedNodeId
        ? nodesWithDragState.find(n => n.id === draggedNodeId)
        : null,
  ...
  return (
    ...
    >
      ...
      {selectedNodeId && (
        <div
          className={`absolute bottom-4 right-4 flex gap-4 z-10 transition-opacity duration-300 ${
            isAnimatingToPanel || showGenerativePanel
      ...
      {selectedNodeId && (
        <div
          className={`absolute bottom-0 left-0 right-0 bg-gray-800 border-t border-gray-700 p-4 z-20 transform transition-transform duration-300 ease-out ${
      ...
      <GeneratingBlocker
        ...
        status={
          generationError || (isStreaming ? stats.status : 'Generating...')
        }
