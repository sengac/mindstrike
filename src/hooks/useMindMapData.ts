...
export function useMindMapData(
  ...
  useEffect(() => {
    if (initialData && initialData.root) {
      ...
      setTimeout(() => {
        const { nodes: convertedNodes, rootNodeId: convertedRootId, layout: loadLayout } = convertTreeToNodes(initialData)
        ...
        setHistory([{ nodes: convertedNodes, rootNodeId: convertedRootId, layout: loadLayout }])
        setHistoryIndex(0)
