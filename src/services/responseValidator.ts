...
export class ResponseScanner {
  ...
  static scanForRenderableContent(content: string): RenderableContent[] {
    ...
    const mermaidMatches = this.findMatches(content, /```mermaid\n([\s\S]*?)\n```/g);
    ...
    const codeMatches = this.findMatches(content, /```(\w+)?\n([\s\S]*?)\n```/g);
  ...
  private static findMatches(content: string, regex: RegExp): Array<{
    content: string;
    startIndex: number;
    endIndex: number;
    groups?: string[];
...
export class OffScreenValidator {
   ...
   */
  static async validateContent(item: RenderableContent): Promise<ValidationResult> {
    this.initialize();
  ...
  private static async validateMermaid(content: string): Promise<ValidationResult> {
    try {
      ...
      if (hasValidSVG) {
        ...
        return { success: false, error: 'Mermaid failed to generate valid SVG' };
      }
    } catch (error) {
      return { 
        success: false, 
  ...
  private static async validateLatex(content: string): Promise<ValidationResult> {
    try {
  ...
  private static validateCode(content: string, language?: string): ValidationResult {
      ...
      switch (language.toLowerCase()) {
        ...
        case 'js':
          ...
          if (content.includes('function') && !content.includes('{')) {
            return { success: false, error: 'Function declaration missing opening brace' };
          }
          break;
      }
    }
